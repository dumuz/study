...
<div class="place">
    ...
    <div class="tools">
        ...
        <form action="" method="get">
            ...
            <tr>
                <td>
                    ...
                    <select name="classId" style="border:2px solid #F0F0F0; height:34px;"
                    class="xuanz">
                        <option value="">搜索全部</option>
                        <?php $list=lists(2); ?>
                        {volist name="list" id="row"} 
                        <option value="{$id1=$row.id}">{$row.sortname}</option>{/volist}
                    </select>
        ...
        <a href="/admin/ht/news/add"><li class="click"><span><img src="/images/images/t01.png" /></span>添加</li></a>
        </ul>
    ...
    <form action="/admin/ht/news/dels" method="post"> 
    <table class="tablelist">
        ...
        <tr>
            ...
            <td>{$row.sortname}</td>
            <td>{$row.title}</td>
            <td style='text-align:center;text-indent:0px;padding-top:10px;'><img src='__PUBLIC__uploads/{$row.pic}' style='width:180px;'/></td>
            <td>{$row.views}</td>
            <td>{:date("Y-m-d H:i:s",$row.addtime)}</td>
            <td>
                <a href="{:url('/admin/ht/news/edit',['id'=>$row.id])}" id="jbb">编辑</a>&nbsp&nbsp
                <a href="{:url('/admin/ht/news/dels',['id'=>$row.id])}" id="jbb">删除</a>
            </td>
        </tr>
        {/volist}
        </tr>
        ...
        $("#dels").on("click",function(){
            ...
            $.ajax({
                ...
                success:function(msg){
                    if(msg.msg==1){
                         layer.confirm('删除成功', {icon:6,
                            ...
                            $(location).attr('href',"/admin/ht/news/index");
                       });
                    }
                }
            })
                ...
                btn2:function(index,layero){
                    layer.close();
                    window.location.href="{:url('/admin/ht/news/index')}";
                }
            })
          })
    });
